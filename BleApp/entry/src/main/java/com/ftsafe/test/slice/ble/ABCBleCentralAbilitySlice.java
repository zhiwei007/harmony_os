...
public class ABCBleCentralAbilitySlice extends AbilitySlice {
//    ABC
//    private static final String SERVICE_UUID =          "46540001-0001-00e2-0011-465453414645";
    private static final String NOTIFY_CHARACTER_UUID = "46540003-0001-00e2-0011-465453414645";
    private static final String WRITE_CHARACTER_UUID =  "46540002-0001-00e2-0011-465453414645";
//    private static final String DISCIBTOR_CHARACTER_UUID = "00002902-0000-1000-8000-00805f9b34fb";
        ...
        private void checkGattCharacteristic(GattService service) {
            ...
            for (GattCharacteristic tmpChara : service.getCharacteristics()) {
                if (tmpChara.getUuid().equals(UUID.fromString(NOTIFY_CHARACTER_UUID))) {
                    ...
                    /*写描述符 并非必要  */
                  /*  List<GattDescriptor> list =    tmpChara.getDescriptors();
                    for (int i = 0;i<list.size();i++){
                        GattDescriptor gattDescriptor =  list.get(i);
                        HiLog.error(hilabel, " writeDescriptor UUID========>"+gattDescriptor.getUuid().toString());
                        if(DISCIBTOR_CHARACTER_UUID.equalsIgnoreCase(gattDescriptor.getUuid().toString())){
                            HiLog.error(hilabel, " match writeDescriptor UUID========>" );
                            byte[] value = Convection.hexString2Bytes("0100");
                           boolean isSetOk =  gattDescriptor.setValue(value);
                           HiLog.error(hilabel,isSetOk ? "set ok":"set failed!");
                            boolean isOk = peripheralDevice.writeDescriptor(gattDescriptor);
                            if(isOk){
                                HiLog.error(hilabel,"writeDescriptor  write ok!");
                            }else{
                                HiLog.error(hilabel,"writeDescriptor  write failed!");
                            }
                        }
                    }*/
                }
    ...
    private class MyBleCentralManagerCallback implements BleCentralManagerCallback {
        ...
        public void scanResultEvent(BleScanResult bleScanResult) {
          ...
          if(deviceName.contains("农行")){
               peripheralDevice = bleScanResult.getPeripheralDevice();
               updateComponent(deviceText, peripheralDevice.getDeviceName().get());

                List  uuidList  = bleScanResult.getServiceUuids();
                if(uuidList!=null){
                    HiLog.error(hilabel,"ABC getServiceUuids:"+ uuidList.size());
                }
                for (Object uuid:uuidList ){
                    String hexUUID = uuid.toString();
                    HiLog.error(hilabel,"ABC hexUUID:"+ hexUUID);
                }
    ...
    private void initClickedListener() {
        ...
        sendButton.setClickedListener(component -> {
            ...
            byte[] sendata = Convection.hexString2Bytes("611B000000000000000080100000000000000000");
          ...
          try {
            ...
            if (isSetValueOk) {
                sendata = Convection.hexString2Bytes("000000000000000081060000000000006D");
